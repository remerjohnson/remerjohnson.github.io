...
Column Header Name	Description	Internal	
Controlled Values /
Format Restrictions
Object Unique ID
(Required)
An object and its components share the same Object ID. These are usually supplied by the data provider.  It is not mapped to DAMS, but you can copy it in "Identifier:other" if it is useful.
Used by IT to delineate objects from components.
Used by DOMM simply to keep track of which things are objects and components visually.
If not provided, DOMM must create.
